...
Macros =bydi-was-called{-with}= accept an optional argument to clear
the the history for that function (might allow for some easier
chaining in some scenarios).
