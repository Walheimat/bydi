...
In fact, watchers work exactly like mocks and support all verification
macros, but for readability you can use the aliases, that all work
like =bydi-was-{called=>set}-{with=>to}=.
